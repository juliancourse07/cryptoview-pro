...
Agrupa por días, semanas, meses para mejor visualización
"""
import pandas as pd
import numpy as np
...
def aggregate_predictions(predictions_df: pd.DataFrame, aggregation: str = 'daily') -> pd.DataFrame:
    """
    Agrupa predicciones por período
    ...
    Returns:
        DataFrame agrupado con min, max, avg por período
    """
...
def create_calendar_view(predictions_df: pd.DataFrame, current_price: float) -> pd.DataFrame:
    ...
    daily['trend'] = daily['change_pct'].apply(lambda x: '📈' if x > 2 else '📉' if x < -2 else '➡️')
...
def get_milestone_predictions(predictions_df: pd.DataFrame, milestones: list = None) -> dict:
    """
    Obtiene predicciones en hitos específicos (7d, 14d, 30d, etc)
    
    Args:
        predictions_df: DataFrame con predicciones
        milestones: Lista de días para extraer [7, 14, 30, 60, 90, 180, 365]
